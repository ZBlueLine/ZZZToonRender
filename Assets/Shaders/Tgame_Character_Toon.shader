Shader "Tgame/Character/Toon Character"
{
    Properties
    {
        ...
        [NoScaleOffset][Linear]_LightMap("Light Map R:光滑度 G:金属度， B:高光亮度 (面部不需要这张图)", 2D) = "white" {}
        [NoScaleOffset][Linear]_LightMap2("R:金属件遮罩 G:Matcap强度 (面部不需要这张图)", 2D) = "white" {}
        ...
        [NoScaleOffset]_MatCap1("金属 Matcap", 2D) = "black" {}
        ...
        Pass
        {
            ...
            half4 CharacterToonPassFragment(Varyings input) : SV_Target
            {
                ...
                indirectLightColor *= lerp(1, baseColor, _IndirectLightMixBaseColor);
                    ...
                    half3 matcapColor = SAMPLE_TEXTURE2D(_MatCap1, sampler_LinearRepeat, matcapCoords).rgb * matcapStrength; 
